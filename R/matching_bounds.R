...
matching_bounds_cplex <- function(Y, D, constraints, M, Kt, Kc, approximate=TRUE,
                                   cplex_control=list()){
    ...
    max_m = which(matrix(max_mb$xopt, sum(D==1), sum(D==0), byrow=T)==1, arr.ind=T)
    max_te_mb = mean(Y[D==1][max_m[, 1]] - Y[D==0][max_m[, 2]])
    ...
    if(approximate)
      max_mb = approx_int_solution2(args, max_mb$xopt, cplex_control)
    min_m = which(matrix(min_mb$xopt, sum(D==1), sum(D==0), byrow=T)==1, arr.ind=T)
    min_te_mb = mean(Y[D==1][min_m[, 1]] - Y[D==0][min_m[, 2]])
